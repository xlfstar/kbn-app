<template>
  <x-table
    ...
    :api="warehouseTypeApi.getWarehouseTypeList"
    :page-options="true"
    :modal-form-data="modalFormData"
    :modal-api="
      isEdit ? warehouseTypeApi.updateWarehouseType : warehouseTypeApi.createWarehouseType
    "
    :scroll-x="900"
    :multiple-check="true"
    modal-grid-cols="1"
...
import { warehouseTypeApi } from '@renderer/api'
...
const handleDelItem = async (row) => {
  const { id } = row || {}
  try {
    await warehouseTypeApi.removeWarehouseType(id)
    handleRefresh()
  } catch (err) {
...
const handleDeleteItems = (ids) => {
  if (ids.length > 0) {
    warehouseTypeApi.removeWarehouseTypeList({ ids }).then((res) => {
      if (res.success) {
        handleRefresh()
      }
    })
